...
ag --version
ag doctor           # checks this machine and says how to fix what is missing
ag hook install     # optional: the Claude Code hook that says why a session waits
```
...
If you move the clone, re-create the `ag` link and run `ag hook install`
again; it repoints the
[Notification hook](../README.md#the-notification-hook) at the new path.
...
Start with `ag doctor`: it checks each prerequisite and prints the fix.
When reporting a problem, include the output of `ag --version` and `ag doctor`.
